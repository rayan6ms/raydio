...
for e in r['events']:
    ...
    quiet.append({'utc':at.isoformat(),'durationMs':e['durationMs'],'nearestStartOffsetSeconds':offset,'within2SecondsOfTrackStart':offset is not None and abs(offset)<=2})
...
summary={'status':r['status'],'startedAt':r['startedAt'],'receiverSeconds':r['elapsedSeconds'],'coverage':r['coverage'],
    'packetsReceived':d['packetsReceived'],'lostNet':d['packetsLost'],'positiveLoss':r['sampling']['positiveLossDeltas'],'recoveredLoss':-r['sampling']['negativeLossDeltas'],
    ...
    'unexpectedQuietAtLeast100Ms':[q for q in quiet if q['durationMs']>=100 and not q['within2SecondsOfTrackStart']],
    ...
    'sourceTailReferenceMs':938.458,'limitations':['one receiver cannot isolate network hop','short window is not six-hour guarantee','quiet alignment is temporal classification, not fresh decoded waveform alignment','sender/resource counters use bracketing minute samples and cannot prove per-packet send timing']}
if len(samples)>1:
